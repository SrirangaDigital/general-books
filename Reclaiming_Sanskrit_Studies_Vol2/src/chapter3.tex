\chapter[Practice {\sl  versus} Theory: Gaṇita-śāstra and...]{Practice {\sl  versus} Theory: Gaṇita-śāstra and Western Mathematics}\label{chapter3}
...
Pollock's\index{Pollock! approach summarized} approach may be succinctly captured summarized as follows:
\begin{itemize}
...
While discussing Rājaśekhara’s\index{Rajasekhara} enumerations of {\sl śāstra}-s he observes that even Gaṇita\index{Ganita} is ``amenable to treatment in {\sl śāstra}'' and is one of the many practices for which the rules provided by {\sl śāstra} (as per his narrow definition) also apply (Pollock 1985:502). 
...
While discussing the epistemological implications of {\sl satkārya-vāda} Pollock assumes that they ``operated subliminally in the mythic representation of the transcendent provenance and authority of {\sl sastra}'', and in fact, in a Western context the same is seen in the ``Socratic merging of {\sl mathesis} and {\sl anamnesis}'' (Pollock 1985:518). {\sl Satkārya-vāda}\index{Satkaryavada} is out of scope for this paper, but the notion of {\sl mathesis} has a very important role to play in Western mathematics. 
...
%\begin{myquote}
Raju (2011b:276) points out that the 47$^{\text{th}}$ proposition (``Pythagorean theorem'') could easily be solved in one step, like Indian texts, if empirical were used throughout and thus the ``myth (that the Elements concerns metaphysical proof) makes the Elements either trivial or inconsistent.''
%\end{myquote}
...
Indian tradition has been very clear that Gaṇita had wide applications in worldly, Vedic and religious affairs, and its development through experimentations, new inventions, discoveries, and innovation, all stemmed from this all-pervasive need, and not merely as a commentary on the Veda-s. From ancient times to pre-colonial times, Gaṇita\index{Ganita} as a {\sl Śāstra} (science) has relentlessly focused on practical issues - developing logical and efficient algorithms for problem solving, and was viewed as an application-oriented subject by its practitioners. Gaṇita-śāstra\index{sastra@sastra(śāstra)} was practiced even among Jain and Buddhist scholars, who do not regard Vedas as a {\sl pramāṇa}.\index{pramana} It is also believed that geometry\index{geometry} as a discipline, most likely originated from the Gaṇita - like {\sl śāstra} of a culture resembling Vedic culture. Like any scientific discipline, Gaṇita-śāstra also operates on real entities, relies on empirical data and is falsifiable. Based on all the evidence furnished so far, Pollock’s claim - that Indian tradition did not view the discoveries and innovations\index{sastra@sastra(śāstra)!innovations in arithmetic} of Indian arithmetic as scientific achievements and progress, but rather as recovery of past truths - must be categorically rejected.

On the other hand, it is formal mathematics which is based on abstractions, on formalism, on the notion of eternal truths and falsehoods, and on a complete rejection of the empirical world. Unlike science, experimentations and inferences ({\sl pratyakṣa-\index{pratyaksa}pramāṇa} and {\sl anumāna}\index{anumana}) play no role in formal mathematics. Since there is no room for the empirical, there is no question of empirical falsifiability\endnote{Science is falsifiable, i.e., new observations can falsify previous conclusions.}. Pollock’s observation below on {\sl śāstra}-s as a fountainhead of all knowledge, can be applied equally well to modern mathematics as a fountainhead of all truths.
\begin{myquote}
...
Against the mass of evidence we have furnished it is evident that Pollock’s approach as well as chain of reasoning and conclusion are not valid universally for all {\sl śāstra}-s, especially those which rely on empirical knowledge as their fundamental principle. It may be fair to say that what may be applicable to transcendental knowledge may not be applicable to the so-called hard sciences like Gaṇita\index{Ganita} or astronomy\index{astronomy} which are heavily computational in nature. 
