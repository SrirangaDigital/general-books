...
{\sl In the Stanford Encyclopedia\index{Stanford Encyclopedia} of Philosophy (2015) (SEP) entry on Epistemology\index{epistemology} in Classical Indian Philosophy}, (Philips says)
\begin{myquote}
...
The hoary aim of the methods of the Critical Theory\index{Critical Theory} is to ``emancipate'', in a Western sense, the societies and systems under study. Implicit in the definition is that it is only the ``West'' that gets to comment and critique, based on methods created by the West. In the specific case of Indology - most of the Indologists are {\sl not} practising adherents of Indic lifestyles, and do not adhere to or live by {\sl dhārmic} notions of the world, and do not have world views which originate from these alternative (non-Western and {\sl dhārmic}) epistemologies. Is not such a social inquiry made via these methods poisoned by existing biases?
...
Pollock derives the ``power'' of his philology from the Post-modern views of language. The `power discourse'\index{power discourse} that he attempts to `see' in historical text, is also influenced by these Postmodern perspectives.\\[-20pt]
...
At this point, it must be asserted, the logic governing Pollock's philology and the scientific method are poles apart. {\sl A serious re-consideration of such fallacious methods\index{misinterpretation!techniques of!mixing disciplines} by scholars like Sheldon Pollock is necessary}.
...
The first formal definition of {\sl śāstra} as theory is supposedly by the Mīmāmsāka viz. Kumārila-bhaṭṭa\index{Kumarilabhatta@Kumārila Bhaṭṭa}
...
The proverbial elephant in the room is staring right at him - So how does Pollock react? - he does not. He moves on, instead, to cite the rules from Kauṭilya's {\sl Artha-śāstra}.\index{Kautilya@Kauṭilya}\index{Arthasastra@\textsl{\sl Artha-śāstra}}
...
To make the claim that even in this presupposition, theory precedes practice, Śaṅkara's {\sl Bhāṣya} on the {\sl Brahma Sūtra}-s\index{Brahmasutra@\textsl{Brahma-sūtra}-s} is quoted selectively and out of context.\index\index{misinterpretation!techniques of!errors/out-of-context quotation} Pollock now claims 
\begin{myquote}
