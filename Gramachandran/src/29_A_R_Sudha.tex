\documentclass[pra,preprint,a4paper,superscriptaddress]{revtex4}
\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage{graphicx}
...
In his 1932 paper~\cite{majorana} %(which had not received much attention at that time) 
...
\begin{enumerate}
%\item Let us first note that the $2N$ orientation angles $(\alpha_l,\beta_l)$, $l=1,2,\ldots,N$, specifying the spinors (\ref{spinor}) of (\ref{Maj}) %may be conveniently  parametrized  in terms of the  $N$ complex parameters $z_l$ as,  
%\begin{equation}
%\label{z}
%z_l=\tan\frac{\beta_l}{2}e^{i\alpha_l},\ \ l=1,2,\ldots,N
%\end{equation}  
%and   the spinors $\vert\epsilon_l\rangle$ of (\ref{spinor}) may thus be expressed as 
%\begin{equation}
%\label{spinor2}
%\vert \epsilon_l\rangle=A_l\, \left[\vert 0 \rangle + z_l\,\vert 1 \rangle\right], \ \ A_l=e^{-i\alpha_l}\,\cos(\beta_l/2).
%\end{equation} 
