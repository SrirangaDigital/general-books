...
\addtocontents{toc}{\protect\contentsline{section}{{\sl Rajaram Nityananda}\smallskip}{}}

\authinfo{Professor, Azim Premji University, Bengaluru}
...
checked that $\hat{e}^{\ast}_R  \cdot \hat{e}_R = \hat{e}_L \cdot \hat{e}_L = 1$ and $\hat{e}^{\ast}_R \cdot \hat{e}_L = 0$. Any vector can be rewritten
...
Like the world map, this represents a sphere as a rectangle, with 
...
It is a family of straight line segments whose centres lie on a circle.  
...
in our diagram of $S^3$ on page~\pageref{diags3}, it is impossible draw a smooth surface which
...
The gradient is to be taken perpendicular to the ray, and its direction 
