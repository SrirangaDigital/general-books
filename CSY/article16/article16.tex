...
\begin{frame}
Now \eqref{eq-*} say the {\it joint} $pmf$ $P_{X,Y}(x,y)$ is determined by the {\it marginal} $pmf$'s $P_{X}(x)$ and $P_{Y}(y)$ by taking the product.
...
\begin{frame}
So as we expected for the basic example {\it $X$ and $Y$ are not independent.}
...
In the ``real world'' e.g., the newspaper one often hears (reeds) that two quantities are {\it correlated}. This word is often taken to be synonymous with {\it causality}. This is not correct and the difference is extremely important even in reel life. Here are two real word examples of correlations.
\begin{enumerate}
...
$$
There is a {\it shortcut formula} for covariance.
\end{nonumremark}
...
{\it Now we need $E(XY)$} (the hard part)
$$
E(XY)=\sum\limits_{xy}xy \ P(X=x,Y=y)
$$
Trick - We are summing over entries in the matrix times $xy$ so potentially {\it eight} terms.
\end{nonumsolution}
\end{frame}

\begin{frame}
\begin{nonumsolution}[Cont.]
But the four terms from first row don't contribute because $x=0$ so $xy=0$. Also the first term in the second row doesn't  contribute since $y=0$. So there are only {\it three} terms.
\begin{align*}
...
{\it Every term} in the Formula for $E(XY)$ so $E(XY)$ is the sum of nine zeroes so $E(XY)=0$.
