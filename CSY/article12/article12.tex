...
{\it This is the most important lecture in the course.}
\end{frame}
...
\begin{nonumdefinition}
A normal distribution with mean $0$ and variance $1$ (so $\mu=0$ and $\sigma^{2}=1$, so $\sigma=1$) is called a {\it standard normal distribution}.
...
\end{itemize}
The challenge is to use the answer to (a) namely $.8944$ to do (b) and (c). In other words to do all three parts you have to look up {\it only one value}.
...
&\quad =2(.8944)-1=.7888
\end{align*}
So we repeat - {\it all we needed to do all three parts was the one value}
$$
...
\begin{frame}
$z_{\alpha}$ is the number so that the vertical line $z=z_{\alpha}$ cuts off area $\alpha$ to the {\it right} under the graph of $f(z)=\dfrac{1}{\sqrt{2\pi}}e^{-\frac{z^{2}}{2}}$.
...
$$
{\it you have to put in the $dx$ here.}
...
\end{align*}
\end{nonumsolution}
{\it Make sure you understand this computation completely.}
\end{frame}
...
If $X\sim \Bin (n,p)$ and $n$ is large (relative to $p$ and $q$) specifically $np\geq 10$ and $nq\geq 10$ then $X$ is approximately normal. Precisely, if $Y$ is the normal random variable with the same mean and variance as $X$
\end{nonumtheorem}
\end{frame}
...
\begin{nonumproblem}
Professional mathematicians would not accept this theorem. {\it There is no estimate on the error of the approximation.}
...
\end{equation*}
But $X$ is discrete and $Y$ is continuous so $X$ could assign a non-zero probability to the end $a$ and to the other end $b$ whereas $Y$ {\it would assign zero probability to each end.}
...
\begin{frame}
So the right-hand side is {\it too small} so we make it bigger by pushing a $\dfrac{1}{2}$ unit to the left and pushing $b$ $\dfrac{1}{2}$ unit to the right. See the text, pg. 153, - have
$$
...
So we need a {\it number} - If you had a laptop when you took the exam you wouldn't need what comes next.
\end{nonumsolution}
\end{frame}

\begin{frame}
\begin{nonumsolution}[Cont.]
So we use the {\it normal approximation to $X$}. We have 
\begin{align*}
...
\begin{frame}
\begin{nonumsolution}[Cont.]
So we need the {\it one-sided rule of thumb.}
...
$$
So we get lucky, {\it 10 is between $\sigma=7.7$ and $2\sigma=154$}
