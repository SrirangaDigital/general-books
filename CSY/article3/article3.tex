...
\begin{enumerate}
\setcounter{enumi}{1}
\item Let $B$ = there are no hearts $Z$ warning = dangerous, turn 
$$
B\neq A
$$
...
\underline{a straight is almost determined by its bottom (i.e., lowest) card.}
\end{frame}
...
\begin{enumerate}
\item Pick an \underline{ordered} pair of kinds e.g., $J$ or $K$.
...
We get the hand $KKK~JJ$ are $KKK~JJ$ and $JJJ~KK$ different? \underline{yes}, the first beats the second so we were right to pick ordered pairs.
...
In poker ``two pair'' is a hand which consists of 2 of one kind, 2 of a second (\underline{different}) kind and one more of a third kind (\underline{different} from the other two)
...
In 1. do we pick an \underline{ordered} pair or an \underline{unordered} pain?

\underline{Answer} Do the test?

The pair $J$, $K\longrightarrow JJKK$

The pair $K$, $J\longrightarrow KkJJ$

In poker $JJKK$ and $KKJJ$ are the \underline{SAME} so $J$, $K$ and $K$, $J$ give the \underline{SAME} hand so order does not matter.

So we pick an \underline{unordered} pain.
\end{frame}
...
We want the number of \underline{unordered} pairs. It is $\binom{13}{2}=\dfrac{(13)(12)}{2}$
