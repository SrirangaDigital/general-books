...
\begin{frame}
\begin{itemize}
\item[{\huge Z}] $f(x)\neq P(X=x)$ in fact $f(x)$ is not the probability of anything $f$ is a \underline{density} i.e., something you integrate to get the magnitude of a physical quantity.
\end{itemize}
...
Both $\lambda$ and $f$ pre derivatives of ?????? ????? values have meaning.
...
\begin{frame}
Here are some usual properties of \underline{continuous} random variables.
...
\end{itemize}
\underline{end points don't matter.}
\end{nonumtheorem}
\end{frame}
...
\begin{frame}
There will be \underline{three} formulas corresponding to the \underline{two} discontinuities in $f(x)$.
