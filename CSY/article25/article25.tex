...
\begin{frame}
\medskip
\centerline{\includegraphics{figure/art25(1).eps}}

\smallskip

Suppose $X_1, X_2, \ldots, X_n$ is a random sample from a normal population. We have seen that we should use the {\it sample} mean $\overline{X}$ to estimate the population mean $\mu$ and the {\it sample} variance $S^2$ to estimate the population variance $\sigma^2$.
\end{frame}
...
The above statement is on exact  statement but if we take a large sample $(n>30)$ from {\it any} population with mean $\mu$ and variance $\sigma^2$ we may assume
\end{nonumtheorem}
\end{frame}
\begin{frame}
...
\begin{nonumtheorem}
If $X_1, X_2, \ldots, X_n$ is a {\it large} $(n > 30)$ random sample from {\it any} population with mean $\mu$ and variance $\sigma^2$ then 
\begin{itemize}
\item[(i)] $\overline{X} \approx N (\mu , \dfrac{\sigma^2}{n})$

\item[(ii)] $S^2 \approx \chi^2 (n-1)$

\item[(iii)] $\overline{X}$ and $S^2$ are {\it approximately} independent. (then are not independent unless the population is normal.)
\end{itemize}
\end{nonumtheorem}
\end{frame}
