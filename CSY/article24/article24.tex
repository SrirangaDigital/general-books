...
\begin{frame}
Suppose we have $n$ numbers $x_1, x_2, \ldots, x_n$. Then their \underline{squared variation} $sv =sv (x_1, x_2, \ldots, x_n)$ $sv(x_1, x_2, \ldots, x_n) = \sum\limits^n_{i=1} (x_i - \overline{x})^2$


Their \underline{mean} (average) squared variation msv or $\sigma^2_n$ (denoted $\sigma^2$ and called the ``population variance on page 33 of our text) is given by 
$$
...
\begin{nonumtheorem}
\begin{equation*}
sv (x_1, x_2, \ldots, x_n) = \sum\limits^n_{i=1} x^2_i - \frac{1}{n} (\sum\limits^n_{i=1} x_i)^2 \tag{$\ast$}
\end{equation*}
\end{nonumtheorem}
...
\begin{equation*}
sv (x_1, x_2, \ldots, X_n) = a \sum\limits^n_{i=1} x^2_i + b\left(\sum\limits^n_{i=1} x_i \right)^2 \tag{$\ast\ast$}
\end{equation*}
...
\begin{remark}
Any symmetric \underline{quadratic} function $q(x_1, x_2, \ldots, x_n)$ is a linear combination of 
...
\begin{frame}
\myheading{\underline{In Which We Return to Statistics}}
...
Answer - use the \underline{Sample} variance $s^2$ to estimate the \underline{population} variance $\sigma^2$
...
\begin{equation*}
S^2 = \frac{1}{n-1} \sum\limits^n_{i=1} X^2_i - \frac{1}{n(n-1)} \left(\sum\limits^n_{i=1} X_i \right)^2 \tag{b}
\end{equation*}
...
\begin{proof}
\begin{itemize}
\item[(i)] $E(X_i) = \mu$ and $V(Y) =\sigma^2$
