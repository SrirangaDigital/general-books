...
Chips of which $M$ are red and $L=N-M$ are white. Suppose we remove $n$ chips \underline{without replacement} so $n\leq N$. 
...
\begin{frame}
But (*) is harder because we have to consider the case where there are $kMn$ red chip. \underline{So we have to choose $n-k$ white chips as well.}
...
\sharp \left\{
\begin{array}{l}
\text{choices of \underline{exceptly}}\\
k \text{~red chips}\\
\text{in the $n$ chips}
\end{array}\right\}
=
...
\begin{nonumdefinition}
If $X$ is a discrete random variable with $pmf$ defined by page 14 then $X$ is said to have \underline{hyper geometric distribution with parameters $n$, $M$, $N$.} In the text the $pmf$ is denoted 
$$
h(x;n,M,N).
$$
\end{nonumdefinition}
...
\underline{But what about the white chips?} We aren't tone yet, you have to choose $n-k$ white chips and there are only $L$ white chips available so if $n-k>L$ we are sun $k$.
...
\underline{Prediction} (I was wrong before)
...
Suppose you draw $n$ chips \underline{with replacement} and let $X$ be the number of red chips you get. What distribution does $X$ have?
