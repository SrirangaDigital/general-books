...
The fundamental theorem of calculus then says that to evaluate the {\it definite} integral $\int\limits^{b}_{0}f(x)dx$ you take {\it any indefinite} integral, evaluate it at the upper limit $b$ and at the lower limit $a$ and subtract the latter from the former.
\end{frame}

\begin{frame}
Now in two variable calculus you have {\it two} ``partial'' derivatives $\dfrac{\partial f}{\partial x}(x,y)$ and $\dfrac{\partial f}{\partial y}(x,y)$ so you have two partial indefinite integrals $\int f(x,y)dx$ and $\int f(x,y)dy$. 
...
\int (x^{2}+y^{2})dx.
$$
The idea is to {\it tract $y$ as a constant.}
\end{frame}
...
The partial indefinite integral in $x$ is {\it defined only up to a function of $y$.} (because $\dfrac{\partial}{\partial x}g(y)=0$).
...
Note {\it the output is a function of $y$.}
\end{frame}
...
The notation is very confusing because of tradition. Here the $x$-limits are $a$ and $b$ and the $y$-limits are $c$ and $d$ {\it so $a$ and $b$ go with $dx$ and $c$ and $d$ go with $dy$.}

{\it Watch out for this later.}
\end{frame}
...
There are {\it two} ways. It is a famous theorem of Fubini that they lead to the same result. We will see this in our example. {\it Pick the way that leads to the easiest computations.}
\end{frame}
...
\dbend\ Remember $1$ and $2$ go with $dx$ and $0$ and $1$ go with $dy$ (see page 6)
...
{\it However the first step is new it is a partial integral with respect to $x$.}
\end{frame}
...
\item Now we do the partial integration with respect to $y$ so {\it $x$ is a constant.}
\begin{align*}
...
\end{align*}
\dbend\ Most functions of $x$ and $y$ are NOT product functions.
\end{nonumtheorem}
\end{frame}
