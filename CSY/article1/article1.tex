...
$A\cup B$ = ``everything in $s$ that is in either $A$ \underline{or} $B$''

$A\cap B$ = ``everything in $s$ that is in $A$ \underline{and} $B$''
\end{frame}
...
We see that $A\cup B$ is the union of three \underline{mutually exclusive} sets.
$$
...
\begin{enumerate}
\item \underline{include} the singletons $A$, $B$, $C$

\item \underline{exclude} the pairs $A\cap B$, $A\cap C$, $B\cap C$

\item \underline{include} the triple $A\cap B\cap C$
\end{enumerate}
\end{proposition}
\end{frame}
