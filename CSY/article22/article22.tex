...
\myheading{1. \underline{The Election Example}}
\begin{center}
\begin{tabular}{|c|}
\hline\\
Bin (1, ?)\\
...
\begin{frame}
\myheading{2. \underline{The Computer Failure Time Example}}
\begin{center}
\begin{tabular}{|c|}
\hline\\
Exp (?)\\
...
\myheading{3. \underline{The Random Number Example}}
\begin{center}
\begin{tabular}{|c|}
\hline\\
U(0, ?)\\
...
\myheading{\underline{Important Remark}}
...
\medskip
\centerline{\includegraphics{art22(1).eps}}
...
This is \underline{hopelessly naive}. Let's try something else
\end{nonumdefinition}
\end{frame}

\begin{frame}
\myheading{\underline{The Main Problem (some what more precise)}}
...
\medskip
\centerline{\includegraphics{art22(2).eps}}
\end{frame}

\begin{frame}
Now our \underline{function $h$ gives rise to a random variable (statistic)}

$$
W = h (X_1, X_2, \ldots, X_n)
$$
which I will call (for a while) an estimator \underline{statistic}, to distinguish if from the estimator (\underline{number}) $w= h(x_1, x_2, \ldots, x_n)$. Once we have chosen $h$ the corresponding  estimator statistic will ofter be denoted $\hat{\theta}$.
\end{frame}
...
\medskip
\centerline{\includegraphics{art22(1).eps}}
then the \underline{number} $h(x_1, x_2, \ldots, x_n)$ is called the observed value of the estimator statistic $W = h (X_1, X_2, \ldots, X_n)$ on the sample $x_1, x_2,\ldots, x_n$. Unfortunately if too is after denoted $\hat{\theta}$.
...
\begin{equation*}
P (h (x_1, X_2, \ldots, X_n) = \theta) \tag{$\ast\ast$}
\end{equation*}
is maximized 
...
\begin{frame}
because \underline{$h(x_1, \ldots  x_n)$ is a number and $\theta$ is a number} so the above statement amounts to the hopelessly naive criterion from page 5

-choose $h$ so that $h(x_1, x_2, \ldots , x_n) = \theta$.

Now we weaken ($\ast\ast$) to something that can be achieved, in fact achieved surprising easily.
\end{frame}

\begin{frame}
\underline{Unbiased Estimators Mam Problem (fourth version)}

Find an estimator $w = h (x_1,\ldots, x_n)$ so that the expected value $E(W)$ of the estimator statistic $W$ satisfies 
\begin{equation*}
E(W) = \theta \tag{$\ast\ast \ast$}
\end{equation*}

At first glance $(\ast\ast\ast)$ doesn't look much easier to achieve then $(\ast\ast)$ but in fact it is surprising easy to achieve - in fact too easy. There are money $W$ that satisfy $(\ast\ast\ast)$ so we will need further criteria. 
\end{frame}

\begin{frame}
Let's give estimator statistics that satisfy ($\ast\ast\ast$) a name.

\begin{nonumdefinition}
An estimator statistic 

$W = h(X_1, X_2, \ldots, X_n)$ is an \underline{unbiased estimator} of the population parameter $\theta$ of 
$$
E(W) = \theta.
$$
\end{nonumdefinition}

Intuitively ($\ast\ast\ast$) is a good idea but we can make this move precise Various theorems in probability e.g Cheston's requetity.
\end{frame}
...
\begin{frame}
\medskip
\centerline{\includegraphics{art22(3).eps}}
...
\begin{frame}
\underline{Examples of Unbiased Estimators The most important estimation problem}
...
\begin{frame}
\underline{So in both cases the unknown parameter is the population mean $E(X) = \mu$}
...
\begin{frame}
\underline{An Examples Where the ``Common Sense'' Estimator is Biased}

Once we have a \underline{mathematical} criterion for an estimator to be good we will often find to our surprise that ``common sense'' estimators do not meet this criterion. We saw an example of this in the ``Pandemonium jet fighter'' problem 14, on page 242.
...
\begin{frame}
\medskip
\centerline{\includegraphics{art22(5).eps}}
...
Amazingly, if you do page 252, problem 32 \underline{you will see exactly by how much if undershoots the mark}.
...
\underline{\underline{Mathematics trumps common sense}}
\end{frame}

\begin{frame}
\underline{Minimum Variance Unbiased Estimators}
...
\underline{The Principle of Minimum Variance Unbiased Estimation}
