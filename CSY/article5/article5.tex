...
Suppose $A$ and $B$ are \underline{mutually exclusive} events $(A\cap B=\emptyset)$ with $P(A)\neq 0$ and $P(B)\neq 0$
...
In this case if you know $B$ has occurred \underline{then $A$ cannot occur} at all.

This is the \underline{opposite} of independence.
\end{frame}
...
So\quad $P(B|A)\neq P(B)$\quad So $A$ and $B$ are \underline{not} independent.
\end{frame}
...
So $A$ and $B$ \underline{are} independent.
...
\underline{When I give you problems}

\underline{you will told whether or}

\underline{not $A$ and $B$ are independent.}

When we do ``real-life'' problems we here to decide on a model. In this case in example 1. It is clear that we require a model so that $A$ and $B$ are not independent and in example 2 in which they \underline{are}. So we already know the answer to the independence question before doing any mathematics. Again there is something beyond the mathematics.
\end{frame}
...
{\huge Z} To specialize what I said before there are example where $(\sharp)$ holds but one of the $(b)$'s foils so $(\sharp)$ \underline{does not} imply independence.
\end{frame}
...
\begin{frame}
\begin{nonumsolution}
It is important that you follow the format below - \underline{don't skip steps}. Skipping steps is fatal in mathematics (as in almost everything).
\end{nonumsolution}
...
\underline{Express the set $S$ in terms of the sets $A_{1}$, $A_{2}$, $A_{3}$ using the geometry of the system.}
$$
S=A_{1}\cup (A_{2}\cap A_{3})
$$
???? gets $\Leftrightarrow$ $A_{1}$ works \underline{or} (both $A_{2}$ \underline{and} $A_{3}$ work) through.
\end{frame}
...
\begin{frame}
When you form the blocks, the blocks will be independent as long as \underline{on two blocks have a common component}. So in the example choose $A_{1}$ and $C$ are still independent. 
\end{frame}
