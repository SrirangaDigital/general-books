...
$x_{\alpha}$ is the number so that the vertical line $x=x_{\alpha}$ cuts off area $\alpha$ to the \underline{right} under the graph of $f(x)$.

\myheading{Relation between critical values and percentiles}

$x=x_{\alpha}$ cuts off area $1-\alpha$ to the \underline{left} since the total area is 1. But $n(1-\alpha)$ is the number such that $x=\eta(1-\alpha)$ cuts off area $1-\alpha$ to the left.
...
So the point $\eta(p)$ between $a$ and $b$ must have the property that the area of the shaded box is $p$. But the base of the box is $\eta(p)-a$ and the ????? is $\dfrac{1}{h-a}$ so
\begin{gather*}
...
Hence in this \underline{the mean is equal to the median}
$$
...
\begin{itemize}
\item[{\huge Z}] This is not always the case as we will see shortly.
\end{itemize}
\end{frame}
