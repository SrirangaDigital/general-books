...
\begin{frame}
Go to ``BACKGROUND COURSE NOTES'' at the end of my web page and download the file \underline{distributions}.

Today we say goodbye to the elementary theory of probability and start \underline{Chapter 3}. We will open the door to the application of algebra to probability theory by introduction the concept of ``random variable''.
\end{frame}

\begin{frame}
What you will need to get from it (at a minimum) is the ability to do the following \underline{``Good Citizen Problems}''.

I will give you a {\em probability mass function} $P(X)$. You will be asked to compute 
\begin{itemize}
\item[(i)] The \underline{expected value} (or \underline{mean}) $E(X)$.

\item[(ii)] The \underline{variance} $V(X)$.

\item[(iii)] The \underline{cumulative distribution function} $F(x)$.
\end{itemize}
...
\end{tabular}
\end{center}
This is a special case of the \underline{discrete uniform distribution} where $X$ takes values $1,2,3,\ldots,n$ each with probability $\dfrac{1}{n}$ (so roll a fair die with $n$ faces'').
\end{frame}
...
\begin{frame}
The simplest random variable that actually is random takes \underline{TWO} values, let's suppose they are $1$ and $0$ with probabilities $p$ and $q$. Since $X$ has to be either $1$ on $0$ we must have
$$
p+q=1.
$$
So we get
...
\end{tabular}
\end{center}
This called the \underline{Bernoulli random variable with parameter $p$}. So a Bernoulli random  variable is a random variable that takes only \underline{two} values $0$ and $1$.
\end{frame}
...
A \underline{Bernoulli experiment} is an experiment which has two outcomes which we call (by convention) ``success'' $S$ and failure $F$.
\end{nonumdefinition}

\begin{nonumexample}
Flipping a coin. We will call a head a success and a toil a failure.

{\huge Z} Often we call a ``success'' something that is in fact for from an actual success- e.g., a machine breaking down.
\end{nonumexample}
\end{frame}
...
So a \underline{finite} subset of $\mathbb{R}$ is \underline{discrete} but so is the set f integers $\mathbb{Z}$.
\end{nonumdefinition}
\end{frame}
...
\item $P(x)=0$ for all $X$ outside a countable set.
\end{itemize}
\end{nonumdefinition}
\end{frame}
...
E(X) &= (0)\left(\dfrac{1}{8}\right)+(1)\left(\dfrac{3}{8}\right)+(2)\left(\dfrac{3}{8}\right)+(3)\left(\dfrac{1}{8}\right)\\[3pt]
     &= \dfrac{3}{2}
\end{align*}
{\huge Z} The expected value is NOT the most probable value.
\end{nonumexamples}
\end{frame}
...
\end{align*}
The standard deviation $\sigma$ of $X$ is defined to be the \underline{square-root} of the variance
$$
...
The number of arithmetic operations (subtractions) necessary to compute $\sigma^{2}$ can be \underline{greatly} reduced by using.
...
In the formula (*) you need $\sharp(D)$ subtractions (for each $x\in D$ you here to subtract $\mu$ then square ...). For the shortcut formula you need only one. \underline{Always use} the shortcut formula.
\end{nonumproposition}
...
\item[(2)] In the formula for $E(X^{2})$ \underline{don't square} the probabilities
